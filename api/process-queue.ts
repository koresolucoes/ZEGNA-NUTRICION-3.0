        ...
        while (true) {
            ...
            for (const funcCall of functionCalls) {
                ...
                try {
                    ...
                    else if (funcCall.name === 'get_available_slots') { const { data, error } = await supabaseAdmin.rpc('get_available_slots', { p_clinic_id: clinicId, p_target_date: funcCall.args.target_date, p_timezone: clinicTimezone }); if (error) throw error; functionResult = { result: data || [] }; }
                    else if (funcCall.name === 'book_appointment') { const { data, error } = await supabaseAdmin.rpc('book_appointment', { p_clinic_id: clinicId, p_person_id: personData.id, p_start_time: funcCall.args.start_time, p_notes: funcCall.args.notes || null, p_timezone: clinicTimezone }); if (error) throw error; functionResult = { result: data }; }
