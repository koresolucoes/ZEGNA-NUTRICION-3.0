        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--surface-color)', overflow: 'hidden' }}>
            {view === 'categories' ? (
                <>
                    ...
                    <div style={{ padding: '0.5rem 1rem', flex: 1, overflowY: 'auto' }}>
                        <ExpedienteItem 
                ...
                <>
                    <div style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <button onClick={handleBackClick} className="button-secondary" style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem', marginRight: '0.5rem' }}>
                            ← Volver
                        </button>
                        <span style={{ fontSize: '1.2rem' }}>
                            {activeFilter === 'diet' ? '🍐' : activeFilter === 'exercise' ? '📈' : activeFilter === 'consultation' ? '🛡️' : '📁'}
                        </span>
                        <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 600, textTransform: 'uppercase' }}>
                            {activeFilter === 'diet' ? 'PLAN DE ALIMENTACION' : activeFilter === 'exercise' ? 'RUTINA DE EJERCICIO' : activeFilter === 'consultation' ? 'CONSULTA DE SEGUIMIENTO' : 'AUDITORIA'}
                        </h3>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
                        {filteredTimeline.length === 0 ? (
                            <p style={{ textAlign: 'center', color: 'var(--text-light)', marginTop: '2rem' }}>No hay registros en esta categoría.</p>
                        ) : (
