...
// FIX: Renamed clientId to personId for consistency with the new unified schema.
const ConsultationFormPage: FC<{ consultationToEditId: string | null; clientId: string; onBack: () => void;}> = ({ consultationToEditId, clientId: personId, onBack }) => {
        ...
        try {
            ...
            if (consultationToEditId) {
                ...
                const consultationPayload: Database['public']['Tables']['consultations']['Insert'] = {
                    // FIX: Use person_id instead of client_id
                    person_id: personId,
                    consultation_date: formData.consultation_date,
                    weight_kg: weight, height_cm: height, imc, ta: formData.ta || null,
                    notes: formData.notes || null,
            ...
            // Upsert lab results if there's any data
            if (hasLabData) {
            ...
            await supabase.from('logs').insert({
                person_id: personId,
                log_type: 'AUDITORÍA',
                description: `Se ${consultationToEditId ? 'actualizó' : 'creó'} una consulta con fecha ${formData.consultation_date}.`,
    ...
    return (
        <div className="fade-in" style={{ paddingBottom: '7rem' }}>
            <div style={styles.pageHeader}>
                <h1>{consultationToEditId ? 'Editar Consulta' : 'Nueva Consulta'}</h1>
                 <button onClick={onBack} className="button-secondary">{ICONS.back} Volver</button>
            </div>
            <form id="consultation-form" onSubmit={handleSubmit} style={{maxWidth: '700px'}}>
            ...
            <div style={styles.floatingActions}>
                ...
                <button type="submit" form="consultation-form" disabled={loading} style={styles.floatingSaveButton} aria-label={consultationToEditId ? 'Guardar Cambios' : 'Guardar Consulta'}>
                    {loading ? '...' : ICONS.save}
                </button>
            </div>
        </div>
