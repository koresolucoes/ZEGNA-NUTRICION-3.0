...
// FIX: Renamed afiliadoId to personId for consistency
const AfiliadoLogFormPage: FC<{ logToEditId: string | null; afiliadoId: string; onBack: () => void; }> = ({ logToEditId, afiliadoId: personId, onBack }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (logToEditId) {
                ...
                const dataToUpdate: Database['public']['Tables']['logs']['Update'] = {
    ...
    return (
        <div className="fade-in" style={{ paddingBottom: '7rem' }}>
            <div style={styles.pageHeader}>
                <h1>{logToEditId ? 'Editar Bitácora de Afiliado' : 'Agregar a Bitácora de Afiliado'}</h1>
                <button onClick={onBack} className="button-secondary">{ICONS.back} Volver</button>
            </div>
            <form id="afiliado-log-form" onSubmit={handleSubmit} style={{maxWidth: '600px'}}>
            ...
            <div style={styles.floatingActions}>
                ...
                <button type="submit" form="afiliado-log-form" disabled={loading} style={styles.floatingSaveButton} aria-label={logToEditId ? 'Guardar Cambios' : 'Guardar Registro'}>
                    {loading ? '...' : ICONS.save}
                </button>
            </div>
        </div>
...
export default AfiliadoLogFormPage;